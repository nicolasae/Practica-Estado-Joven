...
def run():
    ...
    datos = pd.read_csv(str(settings.BASE_DIR) + "/CSV/ProgramasAcademicos.csv",encoding='utf8')      
    datos = datos.to_json(orient = 'records',force_ascii=False)
    # Se transforma de json a dict de python 
    datos = json.loads(datos)
    ProgramasAcademicos.objects.all().delete()

    for i in datos:
        ProgramasAcademicos.objects.create(**i)
