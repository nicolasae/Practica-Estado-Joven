...
const Graduados = () =>{
    ...
    const getDataMatriculadosList = async () => {
        ...
        var config = {
        method: 'get',
        url: 'http://localhost:8000/api/tendencia?VAR=Graduado',
        headers: { 
            'Content-Type': 'application/json'
        },
        };
    ...
    const getDataTipoInscripcion= async () => {
        ...
        for (var opcion = 0;opcion< opcionesMultiSelect.length;opcion++){
            var config = {
                method: 'get',
                url: 'http://localhost:8000/api/tendencia_count_year?VAR=Graduado&TIPO_INSCRIPCION='+ opcionesMultiSelect[opcion],
                headers: { 
                    'Content-Type': 'application/json'
                },
            };
